...
def run_length_encode(arr):
    ...
    for i, elem in enumerate(arr):
        if i > last_nonzero:
            ...
            values.append(int_to_binstr(elem))#value 1이면 1 -1이면 0
            run_length = 0
            #if size==0:#
            #    print(arr)#
...
def main():
    ...
    npmat = np.array(ycbcr, dtype=np.uint8)#npmat 512x512픽셀의 y cb cr 세 면 존재

    rows, cols = npmat.shape[0], npmat.shape[1] #npmat.shape = (512,512,3)
    ...
    dc = np.empty((blocks_count, 3), dtype=np.int32)#int32인이유는 8개 16진수 저장?
    ac = np.empty((blocks_count, 63, 3), dtype=np.int32)#dc ac안에 쓰레기값만있는거아님?
